...
async def all_messages(message: types.Message):
    try:
        ...
        await ignore(message.chat.id, 1)
        ...
        if message.chat.type == 'private' and str(message.chat.id) in admins:
            if message.reply_to_message:
                if message.reply_to_message.text in [
                    ...
                    if message.content_type in type_permitted:
                        target_type = ''
                        result_broadcast = {}
                        if message.reply_to_message.text == broadcast_text.format('users'):
                            target_type = 'users'
                            result_broadcast = await send_message_broadcast(message, 'users')
                        elif message.reply_to_message.text == broadcast_text.format('grupos'):
                            target_type = 'grupos'
                            result_broadcast = await send_message_broadcast(message, 'grupos')
                        else:
                            return
                        ...
                        msg_result = (
                            ...
                            f' ☆ Inactives: {result_broadcast["inactives"]}\n'
                            f' ☆ Sucess: {result_broadcast["sucess"]}\n'
                            f' ☆ Fail: {result_broadcast["fail"]}'
                        )
        ...
        logger.error(e)
        logger.warning(f'não é possível enviar informações para chat_id={message.chat.id}')
